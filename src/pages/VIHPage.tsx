...
const MainPageContent: React.FC<MainPageContentProps> = ({ resultId }) => {
  ...
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);
  const [value, setValue] = useState<string>(vih?.value ?? "1 350 000 copies/ml; 6.13 log10 ou Pas de virus détecté");
  ...
  useEffect(() => {
    const fetchVih = async () => {
      ...
      try {
        ...
        if (error) throw error;
        setVih(data);
        setValue(data?.value || "1 350 000 copies/ml; 6.13 log10 ou Pas de virus détecté");
      } catch (err: any) {
            ...
            {editMode ? (
              <>
                <Input
                  ...
                  placeholder="ex: 1 350 000 copies/ml; 6.13 log10 ou Pas de virus détecté"
                />
                <p className="text-muted-foreground text-lg">(ex: 1 350 000 copies/ml; 6.13 log10 ou Pas de virus détecté)</p>
                <div className="flex gap-2 mt-2">
                  <Button onClick={handleUpdate} disabled={saving || !value}>
                    {saving ? "Sauvegarde..." : "Sauvegarder"}
                  </Button>
                  <Button variant="outline" onClick={() => { setEditMode(false); setValue(vih.value); }} disabled={saving}>
                    Annuler
                  </Button>
                </div>
              </>
        ...
        <div className="print:block mt-8 text-black text-lg">
          ...
          <div className="flex flex-row justify-between items-start mt-8">
            <div className="flex flex-col items-start gap-2 text-lg" style={{minWidth: '340px'}}>
              <span className="font-semibold">Charge virale&nbsp;&nbsp;VIH</span>
              <span className="text-xs mt-1 whitespace-nowrap">(Abbott m2000rt et m2000sp Technique PCR en temps réel)</span>
              
             { !vih.value.includes('virus') &&  <span className="font-semibold mt-20">Soit</span> }
                ...
                return (
                  <>
                    <span className="font-bold text-2xl">
                      ...
                      <span className="text-xs font-normal"> (limite de détection &lt;40 pour 0.6 ml)</span>
                    <span className="text-xs mt-1">(d'échantillon, 1 copie /ml = 0.58* 1UI/ml)</span>
                    <div className="flex flex-row justify-between w-full mt-8 ">
                      <span></span>
                      <span className="font-bold text-2xl">{matchLog ? matchLog[1] : ''} log<sub>10</sub></span>
                    </div>
                  </>
                );
              })()}
