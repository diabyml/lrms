...
export default function HemocultureMainContent({
  ...
  const [observationModelSearchQuery, setObservationModelSearchQuery] = useState("");
  const [antibiogramModelSearchQuery, setAntibiogramModelSearchQuery] = useState("");
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      setCurrentAntibiogramResultsJson((items) => {
  ...
  // Add function to fetch available ATBs
  ...
  // Add function to handle ATB selection
  const handleSelectAtbs = () => {
    const newAtbs = selectedAtbs.map(atb => ({
      id: uuidv4(),
      name: atb.name,
      order: currentAntibiogramResultsJson.length + selectedAtbs.indexOf(atb),
      s: false,
      i: false,
      r: false,
    }));
    setCurrentAntibiogramResultsJson(prev => [...prev, ...newAtbs]);
    setShowSelectAtbModal(false);
    setSelectedAtbs([]);
    setAtbSearchQuery("");
  };

  // Add effect to fetch ATBs when modal opens
  useEffect(() => {
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center space-x-2">
                        <Input
                          ...
                          value={observationModelSearchQuery}
                          onChange={(e) => setObservationModelSearchQuery(e.target.value)}
                          className="flex-1"
                        />
                      </div>
                      <div className="border rounded-md max-h-[300px] overflow-y-auto">
                        <div className="divide-y">
                          {availableObservationModels
                            .filter(model => 
                              model.name.toLowerCase().includes(observationModelSearchQuery.toLowerCase())
                            )
                            .map((model) => (
                              <div
                                key={model.id}
                                ...
                                onClick={() => {
                                  setSelectedNewObservationModelId(model.id);
                                }}
                              >
                                ...
                                {selectedNewObservationModelId === model.id && (
                      ...
                      <Button
                        onClick={handleApplyNewObservationModel}
                          disabled={!selectedNewObservationModelId || isObservationSaving}
                      >
                        Appliquer
                      </Button>
                      <Button
                        variant="ghost"
                          onClick={() => {
                            setShowObservationModelSelector(false);
                            setObservationModelSearchQuery("");
                            setSelectedNewObservationModelId(undefined);
                          }}
                        >
                          Annuler
                      </Button>
                        ...
                        <div className="p-0">
                          <Table className="min-w-full">
                            ...
                            <TableBody>
                              ...
                              >
                                <SortableContext
                                  items={currentAntibiogramResultsJson.map((item) => item.id)}
                              ...
                              {currentAntibiogramResultsJson
                                ...
                                .map((abItem) => (
                                    ...
                                    key={abItem.id}
                                        item={abItem}
                                        onSirChange={handleAntibiogramSirChange}
                                        onRemove={handleRemoveAdhocAntibiotiqueFromCurrentSet}
                                      />
                                    ))}
                                </SortableContext>
                              </DndContext>
                          ...
                          <Card className="mb-4 p-4 no-print">
                            ...
                            <div className="space-y-4">
                              <div className="flex items-center space-x-2">
                                <Input
                                  placeholder="Rechercher un modèle..."
                                  value={antibiogramModelSearchQuery}
                                  onChange={(e) => setAntibiogramModelSearchQuery(e.target.value)}
                                  className="flex-1"
                                />
                              </div>
                              <div className="border rounded-md max-h-[300px] overflow-y-auto">
                                <div className="divide-y">
                                  {availableAntibiotiqueModels
                                    .filter(model => 
                                      model.name.toLowerCase().includes(antibiogramModelSearchQuery.toLowerCase())
                                    )
                                    .map((model) => (
                                      <div
                                        key={model.id}
                                        className="flex items-center space-x-2 p-2 hover:bg-slate-50 cursor-pointer"
                                        onClick={() => {
                                          if (!appliedAntibiogramSets.some(s => s.source_antibiotique_model_id === model.id)) {
                                            setSelectedNewAntibiotiqueModelId(model.id);
                                          }
                                        }}
                                    >
                                        <div className="flex-1">
                                        ...
                                        {selectedNewAntibiotiqueModelId === model.id && (
                                          <Check className="h-4 w-4 text-green-500" />
                                        )}
                                        {appliedAntibiogramSets.some(s => s.source_antibiotique_model_id === model.id) && (
                              ...
                              <Button
                                onClick={handleSelectAntibiotiqueModelAndStart}
                                  disabled={!selectedNewAntibiotiqueModelId || isLoadingAntibiotiqueData}
                              >
                                Commencer
                              </Button>
                              <Button
                                variant="ghost"
                                  onClick={() => {
                                    setShowAntibiotiqueModelSelector(false);
                                    setAntibiogramModelSearchQuery("");
                                    setSelectedNewAntibiotiqueModelId(undefined);
                                  }}
                              >
                                Annuler
